document.addEventListener('DOMContentLoaded', function() {
    function updateOutput() {
        ...
        const mode = document.querySelector('input[name="textPrefixMode"]:checked').id;
        const skipBlanks = document.getElementById('skipBlankLines').checked;
        let result = '';
        switch (mode) {
          case 'lineByLineMode':
            result = text.split(/\r?\n/).map(line => line.trim() === '' && skipBlanks ? '' : Prefix + line).join('\n');
            break;
          case 'singlePrefixMode':
            result = Prefix + text;
            break;
          case 'paragraphMode':
            result = text.replace(/^(.+)(      |$)/gm, (match, p1, p2) => Prefix + p1 + p2);
            break;
        }
    ...
    // Event listeners for text input and Prefix changes
    document.getElementById('textInput').addEventListener('input', updateOutput);
    document.getElementById('PrefixText').addEventListener('input', updateOutput);
    const modeRadioButtons = document.querySelectorAll('input[name="textPrefixMode"]');
